...
(async () => {
  ...
  // Solicitar el porcentaje de descuento para mascotas Vip
  const descuentoVip = parseFloat(await preguntar("Ingrese el descuento para mascotas VIP (en %): "));
  ...
  for (let i = 0; i < numeroVuelos; i++) {
    ...
    for (let j = 0; j < numeroPasajeros; j++) {
      ...
      if (tieneMascota) {
        ...
        const Vipmascota = (await preguntar(`¿Es la mascota ${j + 1} VIP? (si/no): `)).toLowerCase() === 'si';
  ...
  while (actualVuelo) {
    ...
    while (actualPasajero) {
      ...
      const costo = pasajero.calcularCosto(descuentoVIP, pesoLimite, costoPerro, costoGato, descuentoVip);
      ...
      if (mascotas.Vip) {
      totalDescuentos -= (descuentoVip / 0.15) * pasajero.tarifa;
      }
  ...
  console.log("Dinero total acumulado por descuento de mascotas VIP")
